...
Router::Router(short **map, int w):heap(100000), width(w)
{
...
void Router::findRoutes(const CityPos *cityPos, int cityCount, Edge *paths, int &pathCount)
{
  ...
  while(connectedCount < cityCount)
  {
    ...
    for(int i = 0; i < top; i++)
    {
      ...
      Dij[stack[i].x][stack[i].y].cost = INT_MAX;
    }
    ...
    for(int i = 0; i < cityCount; i++)
    {
      ...
      if( Dij[cityPos[i].x][cityPos[i].y].cost == INT_MAX)
