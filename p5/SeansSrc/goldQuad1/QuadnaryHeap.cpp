...
using namespace std;
        ...
        void BinaryHeap<Comparable>::decreaseKey( const Comparable & x )
        {
          ...
          for( ; pos > 1 && x.cost < array[ (pos + 2) / 4 ].cost; pos = (pos + 2) / 4 )
          {
...
/* 4*/          start = child = hole * 4 - 2;
                if(start + 2 < currentSize)
                {
/* 5*/            if(array[ child + 1 ].cost < array[ child ].cost )
/* 6*/              child++;
/* 5*/            if(array[ start + 2 ].cost < array[ child ].cost )
                   child = start + 2;
                  if(array[start + 3].cost < array[child].cost)
                   child = start + 3;
                }
                else
                  if(start + 1 < currentSize)
                  {
                    if(array[ child + 1 ].cost < array[ child ].cost )
/* 6*/                child++;
/* 5*/              if(array[ start + 2 ].cost < array[ child ].cost )
                      child = start + 2;
                  }
