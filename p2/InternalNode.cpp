...
void InternalNode::removeChild(BTreeNode* child)
{
  int i;
  for (i = 0; i < count && child != children[i]; ++i);
...
void InternalNode::borrowFromRightInter()
{
  BTreeNode* rightMinChild = children[count - 1]->getRightSibling();
  rightMinChild->setParent(this);
  keys[count] = rightMinChild->getMinimum();
  children[count] = rightMinChild;
  // cout << "new child min = " << rightMinChild->getMinimum() << endl;
  count++;
  ((InternalNode*)rightSibling)->removeChild(rightMinChild);
}//

void InternalNode::borrowFromLeftInter()
{
  BTreeNode* LeftMaxChild = children[0]->getLeftSibling();
  LeftMaxChild->setParent(this);
  count++;
  ...
  keys[0] = LeftMaxChild->getMinimum();
  children[0] = LeftMaxChild;
  
  ((InternalNode*)leftSibling)->removeChild(LeftMaxChild);
}//

void InternalNode::mergeToRightInter()
{
...
void InternalNode::mergeToLeftInter()
{
  for (int i = 0; i < count; ++i)
...
{  // to be written by students
  int pos = -1; // will be where value belongs
  BTreeNode *ptr = NULL;
  ...
  if (count >= 0 && count < (internalSize + 1 ) / 2)
  {
    if (leftSibling)
    {
  ...
  if (!leftSibling && !rightSibling && count == 1)
  {
